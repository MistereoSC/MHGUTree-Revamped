...
import RAW_DATA from '@/json/db.json'
...
import WeaponItemElem2 from '@/components/trees/WeaponItemElem2.vue'
...
const items = RAW_DATA as unknown as TWeaponCluster<TDualbladesData>[]
...
function matchesFilter(
	...
	// ELEM >= X: passes if either element meets the threshold
	if (f.minElemAtk > 0) {
		if (
			weapon.data.element_attack < f.minElemAtk &&
			weapon.data.element_2_attack < f.minElemAtk
		)
			return false
		return true
	}

	// Element selection: passes if either slot matches
	if (f.element !== 'any') {
		if (f.element === 'none') {
			// Both slots must be empty
			if (weapon.data.element !== '' || weapon.data.element_2) return false
		} else {
			const elem1match = weapon.data.element.toLowerCase() === f.element
			const elem2match = weapon.data.element_2
				? weapon.data.element_2.toLowerCase() === f.element
				: false
			if (!elem1match && !elem2match) return false
		}
	}
...
<template>
	<div class="relative h-screen">
		...
		<div class="overflow-auto h-full scrollable py-4 px-8">
			...
			>
				<Icon icon="mdi:magnify" class="text-4xl" />
				...
				<LazyRender min-height="200px" @revealed="anyRevealed = true">
					...
					<WeaponTree :cluster="cluster">
						<template #default="{item}">
							<!-- prettier-ignore -->
							<WeaponItemElem2 :weapon="(item as unknown as TWeapon<TDualbladesData>)" :dimmed="isFilterActive && !matchedIds?.has(item.id)" />
						</template>
					</WeaponTree>
				</LazyRender>
			</div>
