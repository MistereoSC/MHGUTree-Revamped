...
import WeaponItemShelling from '@/components/trees/WeaponItemShelling.vue'

const items = RAW_DATA as unknown as TWeaponCluster<TGunlanceData>[]
...
function matchesFilter(
	...
	if (f.shellingType !== 'any') {
		if (!weapon.data.shelling_type.toLowerCase().startsWith(f.shellingType))
			return false
	}
				...
				<LazyRender min-height="200px" @revealed="anyRevealed = true">
					...
					<WeaponTree :cluster="cluster" :node-height="104">
						<template #default="{item}">
							<!-- prettier-ignore -->
							<WeaponItemShelling :weapon="(item as unknown as TWeapon<TGunlanceData>)" :dimmed="isFilterActive && !matchedIds?.has(item.id)" />
						</template>
					</WeaponTree>
				</LazyRender>
			</div>
