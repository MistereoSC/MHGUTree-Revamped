...
function matchesFilter(
	weapon: TWeapon<THuntingHornData>,
	f: TWeaponFilter
): boolean {
	...
	if (f.notes.length > 0) {
		if (!f.notes.every((n) => weapon.data.notes.includes(n))) return false
	}
...
<template>
	<div class="relative h-screen">
				...
				<LazyRender min-height="200px" @revealed="anyRevealed = true">
					...
					<WeaponTree :cluster="cluster" :node-height="104">
						<template #default="{item}">
							<!-- prettier-ignore -->
							<WeaponItemHorn :weapon="(item as unknown as TWeapon<THuntingHornData>)" :dimmed="isFilterActive && !matchedIds?.has(item.id)" />
						</template>
					</WeaponTree>
				</LazyRender>
			</div>
		...
		<div class="absolute bottom-6 left-4">
			<WeaponFilterPanel show-notes @update:filter="currentFilter = $event" />
		</div>
	</div>
</template>
